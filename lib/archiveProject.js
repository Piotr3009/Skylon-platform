...
export async function archiveProject(projectId, archivedBy) {
  try {
    ...
    // KROK 1.5: Walidacja - sprawdź czy wszystkie taski są completed i mają rating
    const { data: preCheckCategories } = await supabase
      .from('categories')
      .select('id')
      .eq('project_id', projectId)

    if (preCheckCategories && preCheckCategories.length > 0) {
      const { data: allTasks } = await supabase
        .from('tasks')
        .select('id, name, status, assigned_to')
        .in('category_id', preCheckCategories.map(c => c.id))

      if (allTasks && allTasks.length > 0) {
        // Check if all tasks with assigned subcontractors are completed
        ...
        if (incompleteTasks.length > 0) {
          ...
          return {
            success: false,
            message: `Cannot archive: ${incompleteTasks.length} task(s) not completed: ${names}. Mark them as done first.`
          }
        }

        // Check if all assigned tasks have ratings
        const assignedTasks = allTasks.filter(t => t.assigned_to)
        if (assignedTasks.length > 0) {
          const { data: existingRatings } = await supabase
            .from('task_ratings')
            .select('task_id')
            .in('task_id', assignedTasks.map(t => t.id))

          const ratedTaskIds = new Set(existingRatings?.map(r => r.task_id) || [])
          const unratedTasks = assignedTasks.filter(t => !ratedTaskIds.has(t.id))
          
          if (unratedTasks.length > 0) {
            const names = unratedTasks.map(t => t.name).join(', ')
            return {
              success: false,
              message: `Cannot archive: ${unratedTasks.length} task(s) without rating: ${names}. Rate subcontractors first.`
            }
          }
        }
      }
